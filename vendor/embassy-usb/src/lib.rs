...
impl<'d, D: Driver<'d>> Inner<'d, D> {
    ...
    fn handle_control_out(&mut self, req: Request, data: &[u8]) -> OutResponse {
        ...
        match (req.request_type, req.recipient) {
            (RequestType::Standard, Recipient::Device) => match (req.request, req.value) {
                ...
                // Allow special devices to handle standard device requests the
                // core stack does not know about, such as SET_FEATURE(DEBUG_MODE)
                // for USB 2.0 EHCI debug devices.
    ...
    fn handle_get_descriptor<'a>(&'a mut self, req: Request, buf: &'a mut [u8]) -> InResponse<'a> {
        ...
        match dtype {
            ...
            // Allow class implementations to provide special standard
            // descriptors, such as USB_DT_DEBUG (0x0a) for EHCI debug devices.
            _ => self.handle_control_in_delegated(req, buf),
        }
    }
}
